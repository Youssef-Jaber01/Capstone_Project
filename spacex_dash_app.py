...
# Callback function to update the scatter chart based on the selected launch site and payload range
@app.callback(
...
def update_scatter_chart(selected_site, payload_range):
    if selected_site == 'ALL':
        ...
        fig.update_layout(title='Payload vs Launch Success (All Sites)')
    else:
        filtered_df = spacex_df[(spacex_df['Launch Site'] == selected_site) &
                                (spacex_df['Payload Mass (kg)'] >= payload_range[0]) &
                                (spacex_df['Payload Mass (kg)'] <= payload_range[1])]
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category')
        fig.update_layout(title=f'Payload vs Launch Success for {selected_site}')
    return fig
